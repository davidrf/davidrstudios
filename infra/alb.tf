...
resource "aws_lb_listener" "https" {
  ...
  certificate_arn   = data.terraform_remote_state.infra_shared.outputs.primary_acm_certificate_arn
...
resource "aws_lb_target_group" "app" {
  ...
  port        = 3000
  protocol    = "HTTP"
  vpc_id      = data.terraform_remote_state.infra_shared.outputs.vpc_id
  target_type = "ip"
  health_check {
    path = "/"
    port = "3000"
